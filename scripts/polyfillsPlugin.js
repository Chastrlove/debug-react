...
function getModulePath(mod, useAbsolutePath) {
  const modPath =
    mod === "regenerator-runtime" ? "regenerator-runtime/runtime" : `core-js/modules/${mod}`;
  return useAbsolutePath ? require.resolve(modPath) : modPath;
}

function createImport(path, mod, useAbsolutePath, polyfillAll) {
  if (polyfillAll) {
...
module.exports = ({ types }, { polyfillAll, polyfills, entryFiles = [], useAbsolutePath }) => {
  let userPolyfills;
  if (polyfillAll) {
    userPolyfills = ["core-js/stable", "regenerator-runtime/runtime"];
  } else {
    userPolyfills = getPolyfills(undefined, polyfills, {
    ...
    visitor: {
      Program(path, state) {
        ...
        userPolyfills
          ...
          .reverse()
          .forEach((p) => {
            createImport(path, p, useAbsolutePath, polyfillAll);
          });
      },
    },
  };
